...
// if going for accuracy, would return Result
// but assume that the tape is correct
...
pub fn run_tape_pt1(tape: &mut Vec<i32>) -> i32{
    let mut i = 0;
    loop {
        ...
        match code[0]{
            ...
            4 => {
                let dest = tape[i + 1] as usize;
                println!("opcode 4 out: {}", tape[dest]);
                i+= 2;
            },
            99 => break,
            _ => {
...
#[aoc(day5, part1)]
pub fn day5(tape: &Vec<i32>) -> String {
    let mut tape = tape.clone();
    run_tape_pt1(&mut tape).to_string()
}
...
#[aoc(day5, part2)]
pub fn day5_pt2(tape: &Vec<i32>) -> String {
    let mut tape = tape.clone();
    run_tape_pt2(&mut tape).to_string()
}
